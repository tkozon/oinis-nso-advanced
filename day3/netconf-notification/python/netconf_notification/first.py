...
def get_device_name(input_string):
    pattern = r'device\{([^}]+)\}'
    match = re.search(pattern, input_string)
    if match:
        return match.group(1)
    else:
        raise ValueError("Device name not found in the input string.")

class HandleNCConfigNotificationFirst(Action):
    @Action.action
    def cb_action(self, uinfo, name, kp, input, output, trans):
        ...
        self.log.info(f"============================")
        device_name = get_device_name(input.path)
...
class Main(ncs.application.Application):
    ...
    def setup(self):
        ...
        self.log.info('HandleNCConfigNotificationFirst RUNNING')
        self.register_action('handle-nc-config-notification-first', HandleNCConfigNotificationFirst)

    def teardown(self):
        # When the application is finished (which would happen if NCS went
        # down, packages were reloaded or some error occurred) this teardown
        # method will be called.
        self.log.info('HandleNCConfigNotificationFirst FINISHED')
